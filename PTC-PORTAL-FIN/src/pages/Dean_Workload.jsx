...
import { 
  ...
  Users, BookOpen, Plus, Trash2, Library
} from 'lucide-react';

// --- MOCK DATABASE DATA ---
// We simulate the data pulling ONLY faculty in the Dean's department (e.g., IICT)
const MOCK_FACULTY = [
  { user_id: 1, first_name: 'John Kennidy', last_name: 'Abunda', role: 'Faculty', status: 'pending' }, // pending, acknowledged, revision_requested
  { user_id: 2, first_name: 'Angel Cylo G.', last_name: 'Real', role: 'Dean', status: 'acknowledged' },
  { user_id: 6, first_name: 'Mark', last_name: 'Bautista', role: 'Part-Time Faculty', status: 'revision_requested' },
];

const MOCK_SCHEDULES = [
  // John Kennidy's Load (9 Units)
  { schedule_id: 101, faculty_id: 1, subject_code: 'ITE314', subject_name: 'Web Systems', units: 3, section: 'BSIT 3A', days: 'MWF', time: '09:00 AM - 10:00 AM', room: 'Lab 1' },
  { schedule_id: 102, faculty_id: 1, subject_code: 'IT201', subject_name: 'Data Structures', units: 3, section: 'BSIT 2A', days: 'TTH', time: '01:00 PM - 02:30 PM', room: 'Room 302' },
  { schedule_id: 103, faculty_id: 1, subject_code: 'IT301', subject_name: 'Database Mgmt', units: 3, section: 'BSIT 3B', days: 'MWF', time: '10:00 AM - 11:00 AM', room: 'Lab 2' },
  // Angel Cylo's Load (3 Units - Admin reduced load)
  { schedule_id: 104, faculty_id: 2, subject_code: 'CC101', subject_name: 'Intro to Computing', units: 3, section: 'BSIT 1A', days: 'SAT', time: '08:00 AM - 11:00 AM', room: 'Room 101' },
  // Mark Bautista's Load (12 Units)
  { schedule_id: 105, faculty_id: 6, subject_code: 'IT401', subject_name: 'Network Admin', units: 3, section: 'BSIT 4A', days: 'TTH', time: '08:00 AM - 09:30 AM', room: 'Lab 3' },
  { schedule_id: 106, faculty_id: 6, subject_code: 'IT401', subject_name: 'Network Admin', units: 3, section: 'BSIT 4B', days: 'TTH', time: '09:30 AM - 11:00 AM', room: 'Lab 3' },
  { schedule_id: 107, faculty_id: 6, subject_code: 'GE001', subject_name: 'Purposive Comm', units: 3, section: 'BSIT 1C', days: 'MWF', time: '01:00 PM - 02:00 PM', room: 'Room 205' },
  { schedule_id: 108, faculty_id: 6, subject_code: 'GE001', subject_name: 'Purposive Comm', units: 3, section: 'BSIT 1D', days: 'MWF', time: '02:00 PM - 03:00 PM', room: 'Room 205' },
];
...
export default function DeanWorkload({ user }) {
  ...
  // Modal States
  const [activeReview, setActiveReview] = useState(null); // The faculty member currently being reviewed
  const [actionModal, setActionModal] = useState({ isOpen: false, type: null }); // 'acknowledge' or 'request_revision'
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      case 'pending': return <span className="bg-yellow-100 text-yellow-800 px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-widest">Needs Review</span>;
      case 'acknowledged': return <span className="bg-green-100 text-green-800 px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-widest">Acknowledged</span>;
      case 'revision_requested': return <span className="bg-red-100 text-red-800 px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-widest">Revision Requested</span>;
      default: return null;
    }
  };

  // Compute workload for a specific faculty member
  const getWorkloadMetrics = (facultyId) => {
    const assignedSchedules = MOCK_SCHEDULES.filter(s => s.faculty_id === facultyId);
    const totalUnits = assignedSchedules.reduce((sum, sch) => sum + sch.units, 0);
    const totalSections = assignedSchedules.length;
    return { totalUnits, totalSections, schedules: assignedSchedules };
  };
  ...
  const processedFaculty = facultyList.filter(f => {
    const fullName = `${f.first_name} ${f.last_name}`.toLowerCase();
    const matchesSearch = fullName.includes(searchTerm.toLowerCase());
    const matchesStatus = filterStatus === 'all' || f.status === filterStatus;
    return matchesSearch && matchesStatus;
  }).sort((a, b) => {
    let valA = a.last_name.toLowerCase();
    let valB = b.last_name.toLowerCase();
    
    if (sortBy === 'units') {
      valA = getWorkloadMetrics(a.user_id).totalUnits;
      valB = getWorkloadMetrics(b.user_id).totalUnits;
    }
  ...
  const acknowledgedCount = facultyList.filter(f => f.status === 'acknowledged').length;
  const revisionCount = facultyList.filter(f => f.status === 'revision_requested').length;

  // --- ACTIONS ---
  const handleAcknowledge = () => {
    setFacultyList(facultyList.map(f => f.user_id === activeReview.user_id ? { ...f, status: 'acknowledged' } : f));
    // Automated announcement generated securely in the background
    showToast(`Workload acknowledged! An official schedule memo has been sent to ${activeReview.first_name}.`);
  ...
  const handleRequestRevision = () => {
    ...
    setFacultyList(facultyList.map(f => f.user_id === activeReview.user_id ? { ...f, status: 'revision_requested' } : f));
    showToast(`Revision requested and sent to the Registrar.`, 'warning');
    setActionModal({ isOpen: false, type: null });
    setRevisionRemarks('');
    setActiveReview(null);
  };
  ...
  const handleAddSubject = (e) => {
    ...
    const addedSubject = {
      ...newSubject,
      subject_id: Date.now(),
      units: parseInt(newSubject.units, 10)
    };
    setSubjects([...subjects, addedSubject]);
    setNewSubject({ subject_code: '', subject_name: '', units: 3 });
    showToast(`Subject ${addedSubject.subject_code} added to the curriculum successfully.`);
  };

  const handleDeleteSubject = (id) => {
    if (window.confirm("Delete this subject from the curriculum master list?")) {
                  ...
                  <div className="border border-gray-200 rounded-2xl overflow-hidden">
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-gray-100 bg-white">
                        ...
                        {subjects.length === 0 && (
                          <tr><td colSpan="4" className="p-6 text-center text-sm text-gray-500 font-medium">No subjects found in the curriculum.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
          ...
          <div className="p-4 border-b border-gray-200 bg-white flex flex-row items-center gap-3 flex-wrap">
            ...
            <div className="flex gap-2 shrink-0">
              ...
              <div className="px-3 py-1.5 border border-gray-200 rounded-lg flex items-center gap-2 bg-green-50 shrink-0 shadow-sm">
                ...
                <div>
                  <span className="text-sm font-bold text-green-700 leading-none">{acknowledgedCount}</span>
                  <span className="text-[9px] font-black text-green-600 uppercase tracking-widest ml-1.5">Ack'd</span>
                </div>
              </div>
              <div className="px-3 py-1.5 border border-gray-200 rounded-lg flex items-center gap-2 bg-red-50 shrink-0 shadow-sm">
                <AlertTriangle size={14} className="text-red-600" />
                <div>
                  <span className="text-sm font-bold text-red-700 leading-none">{revisionCount}</span>
                  <span className="text-[9px] font-black text-red-600 uppercase tracking-widest ml-1.5">Revisions</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 shrink-0 ml-auto flex-wrap">
              <div className="flex bg-gray-100 p-1 rounded-lg shrink-0">
                ...
                <button onClick={() => setFilterStatus('pending')} className={`px-4 py-1.5 text-xs font-bold uppercase tracking-widest rounded-md transition-all ${filterStatus === 'pending' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Pending</button>
                <button onClick={() => setFilterStatus('acknowledged')} className={`px-4 py-1.5 text-xs font-bold uppercase tracking-widest rounded-md transition-all ${filterStatus === 'acknowledged' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Acknowledged</button>
              </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {processedFaculty.length === 0 ? (
                  ...
                  processedFaculty.map((fac) => {
                    const metrics = getWorkloadMetrics(fac.user_id);
                    
                    return (
                      <tr key={fac.user_id} className="hover:bg-green-50/30 transition-colors group cursor-pointer" onClick={() => setActiveReview(fac)}>
                        <td className="px-8 py-5">
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-0.5">{fac.role}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-5">
                          <div className="text-lg font-black text-gray-800 leading-none">{metrics.totalUnits} <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Units</span></div>
                        </td>
                        <td className="px-6 py-5">
                          <div className="text-sm font-bold text-gray-600 flex items-center">
                            <BookOpen size={16} className="mr-2 text-gray-400" />
                            {metrics.totalSections} Classes
                          </div>
                        </td>
                        <td className="px-6 py-5">
                        ...
                        <td className="px-8 py-5 text-right">
                          <button 
                            className={`px-4 py-2 text-xs font-bold rounded-lg transition-all shadow-sm active:scale-95 ${fac.status === 'pending' ? 'bg-[#0e5c2b] hover:bg-[#0a4720] text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                          >
                            Review Load
                          </button>
                        </td>
  ...
  // ==========================================
  const activeMetrics = getWorkloadMetrics(activeReview.user_id);
  const isPending = activeReview.status === 'pending' || activeReview.status === 'revision_requested';
          ...
          {actionModal.type === 'acknowledge' && (
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 text-center animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
              ...
              <p className="text-sm text-gray-500 mb-8 leading-relaxed">
                By proceeding, you verify that the teaching load ({activeMetrics.totalUnits} Units) assigned to <span className="font-bold text-gray-700">{activeReview.first_name} {activeReview.last_name}</span> is correct and approved for the semester.
              </p>
              <div className="flex gap-3">
                ...
                <button onClick={handleAcknowledge} className="flex-1 py-3.5 px-4 bg-[#0e5c2b] hover:bg-[#0a4720] text-white font-bold rounded-xl shadow-md transition-all active:scale-95">Confirm</button>
              </div>
            </div>
          )}
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <div className="flex items-center gap-3 mb-0.5">
            <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">{activeReview.role} • Dept. of IICT</span>
          </div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight font-serif">{activeReview.first_name} {activeReview.last_name}</h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-black text-[#0e5c2b] leading-none">{activeMetrics.totalUnits} <span className="text-sm font-bold text-gray-400 uppercase">Units</span></p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-black text-gray-800 leading-none">{activeMetrics.totalSections} <span className="text-sm font-bold text-gray-400 uppercase">Sections</span></p>
          </div>
        ...
        <div className="overflow-x-auto flex-1 custom-scrollbar">
          <table className="w-full text-left border-collapse min-w-max">
            ...
            <tbody className="divide-y divide-gray-100">
              {activeMetrics.schedules.length === 0 ? (
                <tr>
                ...
                activeMetrics.schedules.map((sch) => (
